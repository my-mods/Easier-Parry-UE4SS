...
Makes parrying more forgiving in *The Blood of Dawnwalker*. Includes native guard recovery after dodging. Release guard before attacking.

- **2× parry timing window** by default, configurable from **0.1× to 50×**, using the game's difficulty-adjusted baseline.
- Keeps held guard available after a dodge, using the game's native ability lifecycle. Release guard before attacking.
...
Close the game, replace/reinstall the Vortex entry as **Root (game folder)** and deploy. Start the game, load a save, keep LT held, dodge repeatedly and check directional parrying afterward. Separately, release LT, attack, and dodge during the attack. If it fails, release and repress LT once. Exit the game before another launch overwrites `ue4ss/UE4SS.log`.
...
The guard correction targets guard held before a dodge. Release guard before attacking. Dodge retains the game's native attack-interruption, stamina, animation and combat eligibility logic. The native assets match the earlier build reported working for dodge and guard. Package and regression checks pass; confirmation of this replacement and live frame-time validation remain pending.
