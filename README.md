...
Makes parrying more forgiving in *The Blood of Dawnwalker*. Includes native held-guard handling for attacks and dodges. The latest attack handoff correction awaits in-game validation.
...
Close the game, replace/reinstall the Vortex entry as **Root (game folder)** and deploy. Start the game, load a save, keep LT held, test single attacks, repeated attacks and attack/dodge overlaps, then check directional parrying after each action. If it fails, release and repress LT once. Exit the game before another launch overwrites `ue4ss/UE4SS.log`.
