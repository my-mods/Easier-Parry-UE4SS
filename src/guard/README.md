...
Only `GA_Dodge` is shipped in the native container. It adds the existing `Player.Input.BlockTagAbilities` tag to `ActivationOwnedTags`. The engine owns this suppression for the dodge ability’s lifetime and removes it on end or cancellation. Forward dodge checks the native combat state after applying its effect. While the state is dodge (8), it uses the existing OnStateChanged listener and retains suppression until that state ends. A failed start ends immediately. Forward completion does not remove invincibility tags it never granted; cancellation unregisters the listener even when the invincibility flag is false. Stamina checks, animation selection, effect application and stock guard input handling are unchanged.
...
Forward completion uses the native GetCurrentState getter and existing state-change and ability-end paths. It adds no guard bytecode, Lua hooks, timers or object searches. It leaves stock guard listener lifetimes intact. It conflicts with other replacements of `GA_Dodge`; a separate mod replacing guard input can change its response to the suppression tag.
